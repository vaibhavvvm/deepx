"""Health checks and the tool-calling capability probe (``autodev doctor``)."""

from __future__ import annotations

from autodev.providers.base import ProviderSpec, get_provider, resolve_model
...
def check_docker_roundtrip(image: str = "python:3.12-slim") -> Row:
    ...
    try:
        ...
        out = client.containers.run(
            image,
            command=["/bin/sh", "-c", "echo autodev-ok > /tmp/probe && cat /tmp/probe"],
            remove=True,
        )
        text = out.decode() if isinstance(out, bytes) else str(out)
        ok = "autodev-ok" in text
