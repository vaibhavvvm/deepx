...
Commands (blueprint section 6):

    autodev init          interactive first-run: detect providers, write config
    autodev doctor        health checks + tool-calling capability probe
    autodev models ...    unified model view / pull
    autodev start [P]     interactive REPL against a sandboxed agent
    autodev run "<task>"  non-interactive one-shot (CI/scripts)
    autodev resume <id>   reattach to a checkpointed session
    autodev sessions ...  list past sessions
    autodev sandbox shell raw shell inside the running container
    autodev config ...    read/write layered config
"""
...
from autodev.config import Config, load_config, set_key
from autodev.providers.base import build_spec, get_provider, list_providers
...
def init(
    ...
    from autodev.commands.init_cmd import run_init
...
def doctor(
    ...
    from autodev import doctor as diag
...
def start(
    ...
    from autodev.commands.run_cmd import run_interactive

    overrides = _network_overrides(network, no_network)
    config = load_config(overrides=overrides)
    spec = _resolve_spec(config, model)
    run_interactive(config, spec, initial_prompt=prompt, yolo=yolo)
...
def run(
    ...
    from autodev.commands.run_cmd import run_oneshot

    overrides = _network_overrides(network, no_network)
    config = load_config(overrides=overrides)
    spec = _resolve_spec(config, model)
    code = run_oneshot(config, spec, task=task, yes=yes)
...
def resume(
    ...
    from autodev.commands.run_cmd import run_resume

    config = load_config()
    run_resume(config, session_id, yolo=yolo)
...
def sessions_list():
    ...
    from autodev.session import list_sessions
...
def sandbox_shell(
    ...
    name = f"autodev-{session_id}" if session_id else None
    if not name:
        ui.error("Provide a keep-alive session id (see 'autodev sessions list').")
        raise typer.Exit(1)
...
def config_edit(
    ...
    from autodev.config import GLOBAL_CONFIG_PATH

    config = load_config()
    path = GLOBAL_CONFIG_PATH if global_scope else config.project_root / ".autodev.toml"
    if not path.exists():
