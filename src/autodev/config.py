...
1. Global   -- ``~/.autodev/config.toml``
2. Project  -- ``<repo>/.autodev.toml`` (committed, never contains secrets)
...
GLOBAL_CONFIG_PATH = Path.home() / ".autodev" / "config.toml"
PROJECT_CONFIG_NAME = ".autodev.toml"
...
def find_project_root(start: Path | None = None) -> Path:
    ...
    Prefers the directory containing ``.autodev.toml``; otherwise the nearest
...
def write_project(project_root: Path, data: dict[str, Any]) -> Path:
    """Persist (merge) values into the project ``.autodev.toml``."""
