"""``autodev init`` -- interactive first-run wizard.
...
writes a project ``.autodev.toml``. Secrets go to the keyring, never the file.
"""
...
from autodev.config import write_project
from autodev.providers.base import get_provider
from autodev.session import ensure_gitignore
...
def run_init(project: Path) -> None:
    ...
    if models:
        ...
        ui.warn("No Ollama models found. You can 'autodev models pull qwen2.5-coder:7b' later.")
    ...
    ui.info("Run 'autodev doctor' to verify, then 'autodev start' to begin.")


def _configure_groq() -> None:
    from autodev.providers.groq_provider import GroqProvider, set_api_key
