...
Owns the conversation loop for ``openlocal start`` / ``resume``. Slash commands
...
from openlocal.providers.base import build_spec, list_providers
...
_HELP = [
    ...
    ("/distill", "extract lessons learned from this session and append to AGENTS.md"),
    ("/plugin enable|disable|status <name>", "toggle a plugin mid-session (e.g. web_search)"),
    ("/exit", "quit (also /quit, :q, Ctrl-D)"),
]


class Repl:
    def __init__(self, agent, config, spec, session: SessionMeta, backend, checkpointer, mode: str = "local"):
        self.agent = agent
        self.config = config
        self.spec = spec
    ...
    def _dispatch(self, line: str) -> None:
        ...
        handler = {
            ...
            "/plugin": lambda: self._plugin(arg),
        }.get(cmd)
        if handler is None:
    ...
    def _model(self, arg: str) -> None:
        ...
        self._rebuild_agent()
        ui.print_status(self.spec, self.session.network, self.mode)
    ...
    def _distill(self) -> None:
        ui.info("distilling session into AGENTS.md...")
        run_turn(
            self.agent,
            "Analyze our session history. Identify the specific commands that successfully "
            "built, tested, or ran this project. Also identify any specific architectural "
            "quirks or rules we discovered. Use the edit_file or write_file tool to append "
            "these as permanent 'lessons learned' to the /workspace/AGENTS.md file.",
            self.cfg,
        )
        ui.success("distillation complete. Rules added to AGENTS.md.")

    def _plugin(self, arg: str) -> None:  # noqa: C901
        """Toggle or inspect opt-in plugins mid-session.

        Usage:
          /plugin status             — show all plugins and their state
          /plugin enable web_search  — turn on web search for this session
          /plugin disable web_search — turn it off and rebuild without it
        """
        parts = arg.strip().split(maxsplit=1)
        sub = parts[0].lower() if parts else "status"
        name = parts[1].strip() if len(parts) > 1 else ""

        _KNOWN_PLUGINS = ["web_search"]

        if sub == "status" or not sub:
            table = Table(title="plugins", header_style="bold")
            table.add_column("plugin")
            table.add_column("enabled")
            table.add_column("note")
            ws_cfg = self.config.web_search_plugin if hasattr(self.config, "web_search_plugin") else {}
            ws_on = ws_cfg.get("enabled", False)
            net = self.config.network_policy if hasattr(self.config, "network_policy") else "none"
            note = "active" if (ws_on and net != "none") else ("blocked by network=none" if ws_on else "")
            table.add_row(
                "web_search",
                "[green]on[/green]" if ws_on else "[dim]off[/dim]",
                note,
            )
            ui.console.print(table)
            return
        ...
        if name not in _KNOWN_PLUGINS:
            ui.warn(f"Unknown plugin '{name}'. Known plugins: {', '.join(_KNOWN_PLUGINS)}")
            return
        ...
        if name == "web_search":
            ...
            if sub == "enable":
                if net == "none":
                    ui.warn(
                        "Cannot enable web_search: network=none (air-gap mode).\n"
                        "Restart with --network restricted to allow outbound search."
                    )
                    return
                # Patch the in-memory config and rebuild the agent
                if hasattr(self.config, "data"):
                    self.config.data.setdefault("plugins", {}).setdefault("web_search", {})["enabled"] = True
                ui.success(
                    "[+] web_search plugin enabled for this session.\n"
                    "    Queries will be sanitized (secrets redacted, paths stripped) "
                ...
                if hasattr(self.config, "data"):
                    self.config.data.setdefault("plugins", {}).setdefault("web_search", {})["enabled"] = False
                ui.success("[-] web_search plugin disabled. Reverting to strict local-only mode.")
                self._rebuild_agent()
            else:
                ui.warn(f"Unknown sub-command '{sub}'. Use: /plugin enable|disable|status <name>")

_AGENTS_TEMPLATE = """\
# Project conventions for the OpenLocal agent
