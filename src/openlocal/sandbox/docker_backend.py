...
from openlocal.sandbox.policy import Policy, PolicyDecision

WORKSPACE = "/workspace"
_MAX_OUTPUT_BYTES = 16_000  # token-safe cap before returning to the model
_LOG_DIR = f"{WORKSPACE}/.openlocal/logs"
...
class DockerSandboxBackend(BaseSandbox):
        ...
        if self._client is None:
            ...
            try:
                ...
                raise SandboxUnavailable(
                    ...
                    "Run 'openlocal doctor' for details.\n"
    ...
    def start(self) -> None:
        ...
        if self._container is not None:
            return
        if self._starting:
            return  # prevent infinite recursion via _ensure_log_dir → _raw_exec → _ensure_running → start
        self._starting = True
        try:
            self._start_inner()
        finally:
    ...
    def _start_inner(self) -> None:
        ...
        self._container = self.client.containers.run(
            ...
            user=user,
            environment={"HOME": WORKSPACE, "OPENLOCAL_SESSION": self.session_id or ""},
            ...
            cap_drop=["ALL"],
            labels={"openlocal.session": self.session_id or "", "openlocal": "1"},
            auto_remove=False,
        )
        self._ensure_log_dir()
    ...
    def _ensure_log_dir(self) -> None:
        """Best-effort mkdir inside the container.

        Uses a direct ``exec_run`` instead of ``_raw_exec`` to avoid the
        ``_raw_exec → _ensure_running → start`` cycle that caused infinite
        recursion when Docker exec failed on a freshly-created container.
        """
        if self._container is None:
            return
        try:
            self._container.exec_run(
                cmd=["/bin/sh", "-c", f"mkdir -p {_LOG_DIR}"],
                workdir=WORKSPACE,
            )
        except Exception:
            pass  # best-effort; directory will be created on first log persist
