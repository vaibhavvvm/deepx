...
a last-used timestamp, stored in ``~/.openlocal/telemetry.json``. No prompts, no
...
TELEMETRY_PATH = Path.home() / ".openlocal" / "telemetry.json"
...
def summary() -> dict:
    """Return the collected counters for display (``openlocal config telemetry``)."""
