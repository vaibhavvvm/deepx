...
Each ``openlocal start`` gets a UUID, a LangGraph SQLite checkpointer at
``.openlocal/sessions/<uuid>.db``, and a small JSON metadata record. Because
checkpoints are addressable this also lays the groundwork for a future
``openlocal rewind`` (blueprint section 10) -- we keep the thread id stable and
...
SESSIONS_DIRNAME = ".openlocal/sessions"
...
def ensure_gitignore(project_root: Path) -> None:
    """Make sure ``.openlocal/`` (state, secrets-adjacent) is git-ignored."""
