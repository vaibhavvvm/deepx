...
def _ensure_builtin_providers() -> None:
    ...
    from openlocal.providers import (  # noqa: F401  (import triggers registration)
        groq_provider,
        llamacpp_provider,
        ollama_provider,
    )

    # Entry-point providers (third parties): openlocal.providers group.
    _load_entrypoint_providers()


def _load_entrypoint_providers() -> None:
    """Discover third-party providers via the ``openlocal.providers`` entry point."""
    try:
        from importlib.metadata import entry_points

        eps = entry_points(group="openlocal.providers")
