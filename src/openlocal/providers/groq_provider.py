...
class GroqProvider(Provider):
    ...
    def resolve(self, spec: ProviderSpec):
        try:
            ...
            raise ImportError(
                "Groq support needs the 'groq' extra: pipx install 'openlocal-cli[groq]'"
            ) from exc

        key = get_api_key()
        if not key:
            raise RuntimeError("No Groq API key found. Run 'openlocal init' or set GROQ_API_KEY.")
